...
export default function FormSubmitInput(props: FormSubmitInputProps) {
	...
	return (
		<div className={`mt-6 flex flex-col gap-1 flex-1 ${props.divFormat}`}>
			<button type="submit" disabled={props.pending || props.disabled} className={`self-start border border-gray-300 rounded-md py-1 px-2 hover:bg-stone-100 disabled:opacity-75 disabled:cursor-not-allowed ${props.format}`}>
				{props.pending ? (
					<span className={props.dots ? "animate-dots whitespace-pre" : ""}>
						{loadingLabel}
						{!props.dots && "..."}
					</span>
				) : (
