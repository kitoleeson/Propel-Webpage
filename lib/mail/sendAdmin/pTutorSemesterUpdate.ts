...
import { TutorSemesterUpdateFormValues } from "@/lib/validation/tutorForm/tutorFormSchema";
import { sendEmail } from "..";
import Mail from "nodemailer/lib/mailer";

export default async function sendAdminPendingTutorSemesterUpdateApprovalEmail(pending_tutor_id: number, data: TutorSemesterUpdateFormValues & { tutor_id: number }) {
	const sections = [
		{
	...
	sections.forEach((section) => {
      ...
      `;
		...
		Object.entries(section.fields).forEach(([key, label]) => {
			const value = data[key as keyof TutorSemesterUpdateFormValues];
	...
	const approveUrl = `${baseUrl}/approvePendingTutorSemesterUpdate?id=${pending_tutor_id}`;

	const options: Mail.Options = {
		to: process.env.ADMIN_EMAIL,
		subject: `Pending Tutor Semester Update Request: ${data.gov_first_name} ${data.gov_last_name}`,
		html: `
