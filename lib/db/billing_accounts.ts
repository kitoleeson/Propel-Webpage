...
export const createBillingAccountsRepo = (sql: any, pool: any) => {
	...
	const getByGuardianOwner = async (guardian_id: number, db: any = sql): Promise<DBTypes.BillingAccountsRow[]> => {
		return db`
	...
	const getByStudentOwner = async (student_id: number, db: any = sql): Promise<DBTypes.BillingAccountsRow[]> => {
		return db`
         SELECT * FROM billing_accounts 
	...
	const getAll = async (db: any = sql): Promise<DBTypes.BillingAccountsRow[]> => {
		return db`SELECT * FROM billing_accounts ORDER BY billing_id`;
	};

	const getOwner = async (billing_id: number, db: any = sql): Promise<DBTypes.OwnersRow[]> => {
		return db`
      SELECT 
         ba.billing_id,
         ba.display_name,
   ...
   `;
	...
	const insert = (data: DBTypes.BillingAccounts, db: any = sql): Promise<DBTypes.BillingAccountsRow[]> => {
		return db`
	...
	const removeById = (billing_id: number, db: any = sql): Promise<DBTypes.BillingAccountsRow[]> => {
		return db`
	      DELETE FROM billing_accounts WHERE billing_id = ${billing_id} RETURNING *;
	   `;
	};

	const update = (billing_id: number, data: { display_name: string; email: string }, db: any = sql): Promise<DBTypes.BillingAccountsRow[]> => {
		return db`
	...
	const setFirstInvoice = (billing_id: number, set_to: boolean = false, db: any = sql): Promise<DBTypes.BillingAccountsRow[]> => {
		return db`
         UPDATE billing_accounts
