...
const FormTextInput = (props: FormTextInputProps) => {
	return (
		<div className={`flex flex-col gap-1 flex-1 ${props.divFormat || ""}`}>
			...
			<input id={props.register?.name} className={`border border-gray-300 rounded-md p-1 ${props.format}`} {...props.register} type={props.type} placeholder={props.placeholder} disabled={props.disabled} />
