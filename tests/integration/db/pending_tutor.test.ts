...
describe("Pending Tutor Repository Integration Tests", () => {
	...
	const insertMockTutor = async (overrides = {}): Promise<DBTypes.PendingTutors> => {
		...
		const result = (await db.tutor.insert.insert(mockData))[0];
		return { ...result, subjects_json: subjectPlaceholder, created_at: new Date() };
	};
	...
	describe("Insert & Find Operations", () => {
		...
		it("should insert a tutor update and retrieve their tutor ID by pending tutor ID", async () => {
			const tutor = await insertMockTutor();
			const pending_tutor_id = (await db.pending_tutor.insert(tutor))[0].pending_tutor_id;
		...
		it("should insert a new tutor and retrieve their tutor ID by pending tutor ID", async () => {
			const tutor = await insertMockTutor();
			const pending_tutor_id = (await db.pending_tutor.insert({ ...tutor, tutor_id: -1 }))[0].pending_tutor_id;
			const result = await db.pending_tutor.get(pending_tutor_id);
			expect(result[0]).toBeDefined();
			expect(result[0].tutor_id).toEqual(-1);
		});
		...
		it("should insert duplicate pending tutor entries for consecutive updates", async () => {
			...
			expect(pending_tutor1[0]).toBeDefined();
			expect(pending_tutor1[0].tutor_id).toEqual(1);
			expect(pending_tutor1[0].pref_name).toEqual("Janie");
			expect(pending_tutor2[0]).toBeDefined();
			expect(pending_tutor2[0].tutor_id).toEqual(1);
			expect(pending_tutor2[0].pref_name).toEqual("Jane");
		});
	...
	describe("Get Operations", () => {
		it("should fetch a specific tutor by ID", async () => {
			...
			const result = await db.pending_tutor.get(inserted[0].tutor_id);
			expect(result[0].gov_first_name).toBe("Jane Catherine");
	...
	describe("Delete Operations", () => {
		it("should remove a tutor by ID", async () => {
			...
			await db.pending_tutor.remove(inserted[0].pending_tutor_id);
			const result = await db.pending_tutor.get(inserted[0].pending_tutor_id);
			expect(result.length).toEqual(0);
		});
